...
export async function getDashboardPayload() {
  ...
  if (!sql) {
    return withEnvironmentDetails(demoPayload);
  }
...
export async function getCameraSources() {
  const sql = getSqlClient();
  if (!sql) {
    return demoPayload.cameraSources;
  }
...
export async function getNotificationSettings() {
  const sql = getSqlClient();
  if (!sql) {
    return withTwilioDetails(demoPayload.notificationSettings);
  }
  ...
  try {
    ...
    return withTwilioDetails(defaultNotificationSettings);
  }
}
...
export async function listAssets() {
  const sql = getSqlClient();
  if (!sql) {
    return demoPayload.assetRegistry;
  }
