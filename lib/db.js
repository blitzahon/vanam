...
export async function getDashboardPayload() {
  ...
  if (!sql) {
    if (canUseLocalRuntime()) {
      ...
      try {
        localPayload = await getLocalDashboardPayload();
      } catch {
        localPayload = null;
      }
...
export async function listAssets() {
  let projectAssets = [];
  try {
    projectAssets = await listProjectAssets();
  } catch (error) {
    console.error("[listAssets] listProjectAssets failed:", error);
  }
  const sql = getSqlClient();
  if (!sql) {
  ...
  try {
    ...
    `;
    return mergeAssetCollections(rows.map(hydrateAsset), projectAssets, canUseLocalRuntime());
  } catch {
...
function hydrateSummary(row) {
  return {
    totalEvents: asSafeCount(row.totalEvents),
    animalEvents: asSafeCount(row.animalEvents),
    accidentEvents: asSafeCount(row.accidentEvents),
    activeCameras: asSafeCount(row.activeCameras),
...
export function getDashboardErrorPayload(errorMessage) {
  return withEnvironmentDetails({
    ...createNeedsSetupPayload(),
    error: typeof errorMessage === "string" ? errorMessage : "Server error while loading the dashboard."
  });
}
...
async function getLocalDashboardPayload() {
  try {
    ...
    console.error("[getLocalDashboardPayload]", error);
    return null;
  }
}
...
function mergeAssetCollections(primaryAssets, secondaryAssets, preferLocalProjectAssets = false) {
  const seen = new Set();
  const merged = [];

  const combined = preferLocalProjectAssets
    ? [...secondaryAssets, ...primaryAssets]
    : [...primaryAssets, ...secondaryAssets];
