...
export async function getNotificationSettings() {
  const sql = getSqlClient();
  if (!sql) {
    return withTwilioDetails(await getLocalNotificationSettings());
  }
...
export async function listAssets() {
  ...
  const sql = getSqlClient();
  if (!sql) {
    return projectAssets.length ? projectAssets : demoPayload.assetRegistry;
  }
...
export async function getAssetById(assetId) {
  const sql = getSqlClient();
  if (!sql) {
    return getProjectAssetById(assetId);
  }
